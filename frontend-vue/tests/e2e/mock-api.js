...
const securityTypes = [
  {
    _id: 'type-1',
    title: [{ key: 'en', value: 'Security Guard Management System Administration' }],
    description: [{ key: 'en', value: 'SecurityGuardManagementSystem security type' }],
    state: true
  }
];
...
const securityMenus = [
  {
    ...
    path: '/dashboard',
    type: { _id: 'type-1', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] },
    source: 'managed',
    state: true
  },
  {
...
const securityGroups = [
  {
    _id: 'group-1',
    title: [{ key: 'en', value: 'Security Guard Management System Admin' }],
    description: [{ key: 'en', value: 'SecurityGuardManagementSystem admin group' }],
    visibleType: { _id: 'type-1', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] },
    state: true
  }
];
...
const securityPermissions = [
  {
    _id: 'perm-1',
    group: { _id: 'group-1', title: [{ key: 'en', value: 'Security Guard Management System Admin' }] },
    menu: { _id: 'menu-1', title: [{ key: 'en', value: 'Dashboard' }], path: '/dashboard', type: { _id: 'type-1', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] } },
    all: true,
    view: true,
    edit: true,
    delete: true,
...
const accountGroups = [
  { _id: 'securityguardmanagementsystem-group-admin', label: 'Security Guard Management System Admin' },
  { _id: 'securityguardmanagementsystem-group-ops', label: 'Security Guard Management System Operations' }
];

const accounts = [
  {
    _id: 'securityguardmanagementsystem-account-1',
    code: 'M-0001',
    email: 'owner.securityguardmanagementsystem@example.com',
    status: { key: 'ACTIVE', title: [{ key: 'en', value: 'Active' }] },
    securityGroups: [{ _id: 'securityguardmanagementsystem-group-admin', title: [{ key: 'en', value: 'Security Guard Management System Admin' }] }],
    userinfo: {
...
let emailNotificationSettings = {
  enabled: true,
  appName: 'SecurityGuardManagementSystem',
  appUrl: 'https://securityguardmanagementsystem.example.com',
  from: 'no-reply@securityguardmanagementsystem.example.com',
  fromName: 'SecurityGuardManagementSystem',
  replyTo: 'support@securityguardmanagementsystem.example.com',
  smtp: {
    host: 'smtp.securityguardmanagementsystem.example.com',
    port: 587,
    secure: false,
    user: 'mailer@securityguardmanagementsystem.example.com',
    hasPass: true
  },
  activeTemplates: ['invite', 'twoFa'],
  invite: {
    subject: 'You are invited to SecurityGuardManagementSystem',
    text: 'Welcome to SecurityGuardManagementSystem',
    html: '<p>Welcome to SecurityGuardManagementSystem</p>'
  },
  twoFa: {
    subject: 'Your SecurityGuardManagementSystem verification code',
    text: '123456',
    html: '<p>123456</p>'
  }
};
...
let emailDeliverySettings = {
  key: 'delivery:securityguardmanagementsystem',
  appId: 'securityguardmanagementsystem',
  enabled: true,
  appName: 'SecurityGuardManagementSystem',
  appUrl: 'https://securityguardmanagementsystem.example.com',
  from: 'no-reply@securityguardmanagementsystem.example.com',
...
const securityguardmanagementsystemEmailWorkflowDefinitions = [
  {
    eventKey: 'payment.created',
    legacyType: '',
    title: 'Payment created',
    description: 'SecurityGuardManagementSystem notification when a payment request is created.',
  ...
  },
  {
    ...
    legacyType: '',
    title: 'Payment paid',
    description: 'SecurityGuardManagementSystem notification when a payment is successfully paid.',
  ...
  },
  {
    ...
    description: 'SecurityGuardManagementSystem notification when a payment attempt fails.',
  ...
  },
  {
    ...
    description: 'SecurityGuardManagementSystem notification when a payment refund is completed.',
  ...
  },
  {
    ...
    description: 'SecurityGuardManagementSystem notification when a merchant is approved.',
  ...
  },
  {
    ...
    description: 'SecurityGuardManagementSystem notification when a merchant request is rejected.',
  ...
  },
  {
    ...
    description: 'SecurityGuardManagementSystem notification when a settlement is completed.',
...
const securityguardmanagementsystemSharedWorkflowEventKeys = [
  'account.invite',
  'auth.2fa.request',
...
function emailWorkflowDefinitionsForApp(appId) {
  ...
  const definitions = normalizedAppId === 'securityguardmanagementsystem'
    ? emailWorkflowDefinitions
      .filter(item => securityguardmanagementsystemSharedWorkflowEventKeys.indexOf(item.eventKey) !== -1)
      .concat(securityguardmanagementsystemEmailWorkflowDefinitions)
...
let databaseBackupSettings = {
  ...
  intervalHours: 24,
  retentionCount: 10,
  backupDir: '/var/backups/securityguardmanagementsystem',
...
let databaseBackupRuns = [
  {
    ...
    mode: 'auto',
    status: 'completed',
    databaseName: 'securityguardmanagementsystem-e2e',
    filename: 'security-guard-management-system-securityguardmanagementsystem-e2e-sample.json.gz',
    sizeBytes: 18432,
...
function buildBackupFilePreview(run) {
  ...
  return {
    run: clone(sourceRun),
    metadata: {
      app: 'securityguardmanagementsystem',
      databaseName: sourceRun.databaseName || 'securityguardmanagementsystem-e2e',
...
function requireSecurityGuardManagementSystemAuth(req, res) {
  const token = getSessionToken(req);
  if (!token || token !== 'securityguardmanagementsystem-e2e-token') {
...
function withAudit(payload) {
  return Object.assign({}, payload, {
    ...
    createdByName: 'securityguardmanagementsystem.owner@example.com',
    updatedByName: 'securityguardmanagementsystem.owner@example.com'
  });
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && path === '/api/v1/auth/me') {
    if (!requireSecurityGuardManagementSystemAuth(req, res)) return;
    writeJson(res, 200, {
      data: {
        _id: 'acc-1',
        email: 'securityguardmanagementsystem.owner@example.com',
        role: 'owner'
      }
    });
    return;
  ...
  if (path.indexOf('/api/v1/accounts') === 0) {
    ...
    if (req.method === 'POST' && path === '/api/v1/accounts/invite') {
      readBody(req).then((body) => {
        const item = {
          _id: `securityguardmanagementsystem-account-${Date.now()}`,
    ...
    if (req.method === 'POST' && path === '/api/v1/security/type') {
      readBody(req).then((body) => {
        const item = withAudit(Object.assign({
          _id: `type-${Date.now()}`,
          appId: 'securityguardmanagementsystem',
          source: 'securityguardmanagementsystem',
          description: [],
          state: true
        }, body || {}));
  ...
  if (path.indexOf('/api/v1/setting/') === 0) {
    ...
    if (req.method === 'POST' && path === '/api/v1/setting/database-backup/run') {
      ...
      const run = {
        ...
        status: 'completed',
        databaseName: 'securityguardmanagementsystem-e2e',
        filename: `security-guard-management-system-securityguardmanagementsystem-e2e-${Date.now()}.json.gz`,
        sizeBytes: 20480,
...
server.listen(port, () => {
  process.stdout.write(`SecurityGuardManagementSystem mock API listening on http://127.0.0.1:${port}\n`);
});
