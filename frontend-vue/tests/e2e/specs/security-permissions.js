const { bootstrapAuthenticatedRoute } = require('../support/securityguardmanagementsystem-session')

module.exports = {
  'SecurityGuardManagementSystem security pages load IAM-backed data and save permission matrix rows': function (browser) {
  ...
  'SecurityGuardManagementSystem security menu page keeps newly-created scoped types and menus visible': function (browser) {
    const devServer = process.env.VUE_DEV_SERVER_URL
    ...
    browser
      ...
      .executeAsync(function (done) {
        ...
        app.$store.dispatch('security/menu/createType', {
          title: [{ key: 'en', value: 'Custom SecurityGuardManagementSystem Tools' }],
          description: [{ key: 'en', value: 'Created from SecurityGuardManagementSystem E2E' }],
          state: true
        }).then(function (type) {
          return app.$store.dispatch('security/menu/createMenu', {
            title: [{ key: 'en', value: 'Custom SecurityGuardManagementSystem Tool' }],
            description: [{ key: 'en', value: 'Custom menu created from SecurityGuardManagementSystem' }],
            path: '/custom/securityguardmanagementsystem-tool',
        ...
        browser.assert.ok((result.value.types || []).some(function (name) { return name === 'Custom SecurityGuardManagementSystem Tools' }), 'created type remains visible')
        browser.assert.ok((result.value.menus || []).some(function (path) { return path === '/custom/securityguardmanagementsystem-tool' }), 'created menu remains visible')
      })
      .end()
  }
}
