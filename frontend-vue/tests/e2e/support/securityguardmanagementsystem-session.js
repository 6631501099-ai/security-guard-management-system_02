function bootstrapAuthenticatedRoute(browser, options) {
  var settings = Object.assign({
    route: '/dashboard',
    token: 'securityguardmanagementsystem-e2e-token',
    profile: {
      _id: 'acc-1',
  ...
  return browser.executeAsync(function (bootstrapOptions, done) {
    ...
    function installDispatchShim(app, shimState) {
      ...
      app.$store.__securityguardmanagementsystemE2EState = Object.assign({}, shimState || {});

      if (app.$store.__securityguardmanagementsystemOriginalDispatch) {
        return;
      }

      app.$store.__securityguardmanagementsystemOriginalDispatch = app.$store.dispatch.bind(app.$store);
      app.$store.dispatch = function (type, payload) {
        var currentShimState = this.__securityguardmanagementsystemE2EState || {};
        ...
        return this.__securityguardmanagementsystemOriginalDispatch(type, payload);
      };
    }
