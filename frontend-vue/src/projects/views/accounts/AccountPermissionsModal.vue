...
export default {
  ...
  computed: {
    ...
    sessionRows () {
      return (Array.isArray(this.sessions) ? this.sessions : []).map(item => ({
        ...
        system: item && item.system ? String(item.system) : 'securityguardmanagementsystem',
  ...
  methods: {
    getSystemLabel (system) {
      const normalized = system ? String(system).trim() : 'securityguardmanagementsystem'
      return normalized ? normalized.toUpperCase() : 'SecurityGuardManagementSystem'
    },
    grantLabels (item) {
      const labels = []
