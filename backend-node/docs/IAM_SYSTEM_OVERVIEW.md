# IAM System Overview — SecurityGuardManagementSystem Backend Node.js

> เอกสารนี้สรุปสถาปัตยกรรม วิธีการทำงาน รูปแบบการเขียนโค้ด และข้อกำหนดของระบบ IAM ใน SecurityGuardManagementSystem Backend
...
SecurityGuardManagementSystem Backend ใช้ระบบ IAM (Identity and Access Management) แบบ **Dual Permission Model** ซึ่งสามารถทำงานได้ 2 โหมด:
...
| **Local Fallback** | `PROJECT_PERMISSION_SOURCE=local` | ใช้ฐานข้อมูล MongoDB ของ SecurityGuardManagementSystem เอง |
...
- รองรับ B2B Token สำหรับ SecurityGuardManagementSystem API
...
```
┌─────────────────────────────────────────────────────────┐
│                   SecurityGuardManagementSystem Backend                        │
│                                                          │
│  ┌──────────┐    ┌──────────────┐    ┌───────────────┐  │
...
|-----------|------|---------|
| IAM SDK | `server/integrations/iam/sdk.js` | Factory สร้าง SDK instance สำหรับ SecurityGuardManagementSystem |
...
| B2B Auth | `securityguardmanagementsystem/service/securityguardmanagementsystem_document.js` | ตรวจสอบ B2B Token สำหรับ SecurityGuardManagementSystem API |
...
- สร้าง SecurityGuardManagementSystem-configured IAM SDK instance ด้วย `createSecurityGuardManagementSystemSdk()`
...
| `forwardAccountsList()` | List accounts พร้อม SecurityGuardManagementSystem scope filter |
| `resolveScopedSecurityMetadata()` | กรอง Groups/Menus ตาม SecurityGuardManagementSystem scope |
| `inviteAccountToScope()` | เชิญ user ใหม่เข้า SecurityGuardManagementSystem scope |
| `syncAccountAssignments()` | Sync group memberships |
| `removeAccountFromScope()` | ปิดการเข้าถึง SecurityGuardManagementSystem ทั้งหมดของ account |
...
```
Client → GET /api/v1/securityguardmanagementsystem/documents/...
  → onCheckAuthorization() → ดึง account จาก x-access-token
  → requirePermission('/security-guard-management-system/registry', 'view')
    → normalizePath('/security-guard-management-system/registry')
    ...
    → ตรวจสอบ matrix['/security-guard-management-system/registry']['view'] === true
  → ถ้าผ่าน: next()
...
### B2B Token Flow
```
External System → POST /api/v1/securityguardmanagementsystem/documents/...
  + Authorization: Bearer <token>
  → onCheckB2BAuthorization()
    → introspectToken() → IAM Service
    → ตรวจสอบ active, audience, scope
  → requireScope('security.guard.management.system.registry.write')
    → ตรวจ token.scopes ประกอบด้วย 'security.guard.management.system.registry.write'
  → ถ้าผ่าน: next()
```
...
| `IAM_SDK_CLIENT_ID` | `securityguardmanagementsystem-api` | Client ID สำหรับ IAM |
...
| `PROJECT_PERMISSION_TYPE_TITLE` | `Security Guard Management System Administration` | ชื่อ type ใน IAM |
...
```
security.guard.management.system.registry.read security.guard.management.system.registry.write security.guard.management.system.report.read
