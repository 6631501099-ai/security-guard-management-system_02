...
const { createSecurityGuardManagementSystemIamService } = require('../server/integrations/securityguardmanagementsystem/securityguardmanagementsystem-iam.service');
...
function createIamService() {
  return createSecurityGuardManagementSystemIamService({
...
function buildManagedClientConfig() {
  ...
  const projectCode = firstNonEmpty(project.code, 'securityguardmanagementsystem');
  const projectName = firstNonEmpty(project.name, 'SecurityGuardManagementSystem');
  ...
  return {
    ...
    metadata: {
      ...
      environments: [{
        ...
        healthCheckUrl: managedClientConfig.projectBaseUrl.replace(/\/+$/, '') + '/api/v1/securityguardmanagementsystem/documents/stats',
