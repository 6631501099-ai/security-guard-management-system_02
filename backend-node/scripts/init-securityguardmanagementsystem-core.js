...
const projectName = project.name || process.env.PROJECT_NAME || 'SecurityGuardManagementSystem';
const projectCode = String(project.code || process.env.PROJECT_CODE || 'securityguardmanagementsystem')
  .trim()
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-+|-+$/g, '') || 'securityguardmanagementsystem';
