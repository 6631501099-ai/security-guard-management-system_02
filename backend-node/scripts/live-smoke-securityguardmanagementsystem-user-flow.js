...
function createHttpClient(baseUrl, token) {
  return axios.create({
    ...
    headers: {
      ...
      'user-agent': 'securityguardmanagementsystem-live-smoke/1.0',
      lang: 'en'
    },
...
async function createTemporaryIamSession() {
  ...
  const token = 'securityguardmanagementsystem-live-smoke-' + crypto.randomBytes(16).toString('hex');
  const now = new Date();
  const expiresAtUnix = Math.floor(Date.now() / 1000) + 1800;
  const sessionPayload = {
    version: '1',
    ip: '127.0.0.1',
    device: 'securityguardmanagementsystem-live-smoke-script',
    dateTime: now,
    xAccessToken: token,
    expired_key: expiresAtUnix,
    deviceId: 'securityguardmanagementsystem-live-smoke-device',
    fingerprint: crypto.createHash('sha256').update('securityguardmanagementsystem-live-smoke-device|securityguardmanagementsystem-live-smoke-script').digest('hex'),
    networkKey: '127.0.0',
