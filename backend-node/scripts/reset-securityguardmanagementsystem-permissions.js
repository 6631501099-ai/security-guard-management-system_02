...
const DEFAULT_TYPE_TITLE = securityConfig.permissionTypeTitle || 'Security Guard Management System Administration';
const DEFAULT_GROUP_TITLE = securityConfig.permissionGroupTitle || 'Security Guard Management System Admin';
...
const DEFAULT_MENU_PATHS = (
  Array.isArray(securityConfig.permissionPaths) && securityConfig.permissionPaths.length > 1
    ...
    : [
  securityConfig.permissionRootPath || '/security-guard-management-system/security/permission',
  '/dashboard',
...
function ensureConfig() {
  if (!securityguardmanagementsystemConfig.iamAdmin.baseUrl || !securityguardmanagementsystemConfig.iamAdmin.clientId || !securityguardmanagementsystemConfig.iamAdmin.clientSecret) {
...
async function getAdminToken() {
  ...
  const response = await axios.post(
    `${securityguardmanagementsystemConfig.iamAdmin.baseUrl}${securityguardmanagementsystemConfig.iamAdmin.tokenPath}`,
    {
      grant_type: 'client_credentials',
      client_id: securityguardmanagementsystemConfig.iamAdmin.clientId,
      client_secret: securityguardmanagementsystemConfig.iamAdmin.clientSecret,
      scope: securityguardmanagementsystemConfig.iamAdmin.scope,
      audience: securityguardmanagementsystemConfig.iamAdmin.audience
    },
    {
      timeout: securityguardmanagementsystemConfig.iamAdmin.timeout
    }
  );
...
async function adminRequest(token, method, path, payload, params) {
  try {
    ...
    const requestConfig = {
      method: method,
      url: `${securityguardmanagementsystemConfig.iamAdmin.baseUrl}${securityguardmanagementsystemConfig.iamAdmin.basePath}${path}`,
      headers: {
...
async function ensureType(token) {
  const created = await adminRequest(token, 'post', '/security/type', {
    ...
    description: toLang('SecurityGuardManagementSystem delegated administration type')
  });
  return created.data;
}
...
async function ensureGroup(token, typeId) {
  const created = await adminRequest(token, 'post', '/security/group', {
    ...
    description: toLang('SecurityGuardManagementSystem delegated administration group'),
    visibleType: typeId,
    state: true
  });
  return created.data;
...
async function ensureMenus(token, typeId) {
  ...
  for (const path of DEFAULT_MENU_PATHS) {
    const created = await adminRequest(token, 'post', '/security/menu', {
      title: toLang(path),
      description: toLang(`SecurityGuardManagementSystem permission for ${path}`),
      path: path,
      type: typeId,
      state: true
    });
