...
test('IAM adapter normalizes scopes, audience, and basic authorization', function () {
  ...
  assert.equal(normalizeScope('sample.read sample.read iam.security.write'), 'sample.read iam.security.write');
  assert.equal(normalizeAudience(['securityguardmanagementsystem-api']), 'securityguardmanagementsystem-api');
  assert.equal(
...
test('IAM adapter handles token, profile, and admin operations end-to-end', async function () {
  ...
  assert.equal(profile.audience, 'securityguardmanagementsystem-api');
  ...
  assert.equal(types.data.some(function (item) {
    return item && item._id === 'type-securityguardmanagementsystem';
  }), true);
  ...
  assert.equal(effectivePermissions.data.permissions.includes('securityguardmanagementsystem.accounts.manage'), true);
