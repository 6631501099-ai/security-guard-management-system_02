...
test('email notification templates support env overrides and placeholders', function () {
    withEnv({
        PROJECT_MAIL_APP_NAME: 'SecurityGuardManagementSystem Test',
        PROJECT_APP_URL: 'https://securityguardmanagementsystem.example.test',
        ...
        assert.equal(template.subject, 'Invite user@example.test to SecurityGuardManagementSystem Test');
        assert.equal(template.text, 'Hello Jane Doe https://securityguardmanagementsystem.example.test');
        assert.equal(template.html, '<p>Jane Doe</p><a href="https://securityguardmanagementsystem.example.test">user@example.test</a>');
    });
});
...
test('SecurityGuardManagementSystem email workflow dispatch calls IAM with securityguardmanagementsystem app scope', async function () {
    ...
    const result = await EmailNotifications.dispatchEmailWorkflow({
        eventKey: 'securityguardmanagementsystem.expiring',
        to: 'customer@example.test',
        context: {
            securityguardmanagementsystemNo: 'SecurityGuardManagementSystem-1'
        },
        request: {
            headers: {
                lang: 'en'
