...
const SecurityGuardManagementSystemDocument = require('../models/securityguardmanagementsystem_document.model');
...
function payloadFromBody(body) {
  const payload = {
    securityguardmanagementsystemNo: cleanText(body.securityguardmanagementsystemNo),
...
function buildListQuery(query) {
  ...
  if (q) {
    filter.$or = [
      { securityguardmanagementsystemNo: new RegExp(q, 'i') },
...
exports.list = async function list(query) {
  ...
  const [rows, total] = await Promise.all([
    SecurityGuardManagementSystemDocument.find(filter).sort({ expiryDate: 1, updatedAt: -1 }).skip(skip).limit(limit).lean(),
    SecurityGuardManagementSystemDocument.countDocuments(filter)
  ]);
...
exports.stats = async function stats() {
  ...
  const [total, active, review, expiring, expired] = await Promise.all([
    SecurityGuardManagementSystemDocument.countDocuments({}),
    SecurityGuardManagementSystemDocument.countDocuments({ status: 'active' }),
    SecurityGuardManagementSystemDocument.countDocuments({ status: 'review' }),
    SecurityGuardManagementSystemDocument.countDocuments({
      $or: [
        { status: 'expiring' },
...
exports.create = async function create(body, request) {
  ...
  const missing = ['securityguardmanagementsystemNo', 'title', 'partnerName'].filter(function (field) {
  ...
  const created = await SecurityGuardManagementSystemDocument.create(updateLifecycleStatus(payload));
  return created.toObject();
};

exports.update = async function update(id, body, request) {
  if (!mongoose.Types.ObjectId.isValid(id)) {
    const error = new Error('Invalid SecurityGuardManagementSystem document id');
    error.status = 400;
    throw error;
  }
  ...
  const updated = await SecurityGuardManagementSystemDocument.findOneAndUpdate(
  ...
  if (!updated) {
    const error = new Error('SecurityGuardManagementSystem document not found');
    error.status = 404;
    throw error;
  }
  return updated;
...
exports.remove = async function remove(id) {
  ...
  const result = await SecurityGuardManagementSystemDocument.deleteOne({ _id: new mongoose.Types.ObjectId(id) });
  return { deleted: result.deletedCount || 0 };
};

exports.seedDemo = async function seedDemo(request) {
  const existing = await SecurityGuardManagementSystemDocument.countDocuments({});
  if (existing > 0) {
  ...
  await SecurityGuardManagementSystemDocument.insertMany([
    {
      securityguardmanagementsystemNo: 'SecurityGuardManagementSystem-INT-2026-001',
      ...
      coordinatorName: 'SecurityGuardManagementSystem Coordinator',
      coordinatorEmail: 'securityguardmanagementsystem@mfu.ac.th',
      status: 'active',
      effectiveDate: today,
      expiryDate: monthsFromNow(30),
      renewalNoticeDate: monthsFromNow(27),
      tags: ['academic', 'exchange'],
      notes: 'Initial demo record for validating SecurityGuardManagementSystem workflows.',
      create: actor
    },
    {
      securityguardmanagementsystemNo: 'SecurityGuardManagementSystem-RES-2026-002',
    ...
    {
      securityguardmanagementsystemNo: 'SecurityGuardManagementSystem-EXP-2026-003',
