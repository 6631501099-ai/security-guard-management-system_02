...
test('pay hub email delivery service scopes get requests to securityguardmanagementsystem', async function () {
    let requestOptions = null;
    iamAdminClient.requestAdmin = async function (options) {
        requestOptions = options;
        return { number: 0, code: 20000, data: { appId: 'securityguardmanagementsystem' } };
    };
...
test('pay hub email delivery service scopes update payloads to securityguardmanagementsystem', async function () {
    let requestOptions = null;
    iamAdminClient.requestAdmin = async function (options) {
        requestOptions = options;
    ...
    await emailDeliveryService.onUpdate({
        body: {
            ...
            smtp: {
                host: 'smtp.securityguardmanagementsystem.example.test'
            }
        },
        headers: {
            lang: 'th'
    ...
    assert.equal(requestOptions.data.appId, 'securityguardmanagementsystem');
    assert.equal(requestOptions.data.appName, 'SecurityGuardManagementSystem');
});
