...
test('database backup settings payload clamps scheduler values and keeps storage server-owned', function () {
    const current = {
        ...
        intervalHours: 24,
        retentionCount: 10,
        backupDir: '/srv/securityguardmanagementsystem/backups',
    ...
    assert.equal(payload.backupDir, '/srv/securityguardmanagementsystem/backups');
...
test('database backup run sanitizer hides file path and marks downloadable files', function () {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'securityguardmanagementsystem-backup-test-'));
...
test('database backup collection preview sanitizer exposes names and counts only', function () {
    const payload = databaseBackupService._private.sanitizeCollectionPreview({
        name: 'SecurityGuardManagementSystem_Documents',
        type: 'collection',
        documentCount: '42',
...
test('database backup archive preview summarizes backup file metadata', function () {
    const payload = databaseBackupService._private.buildArchivePreview({
        ...
        mode: 'manual',
        status: 'completed',
        databaseName: 'securityguardmanagementsystem-test'
    }, {
        metadata: {
            ...
            collections: [
                { name: 'users', count: 3 },
                { name: 'securityguardmanagementsystem_documents', count: 4 }
            ]
        },
        collections: {
            users: [{}, {}, {}],
            securityguardmanagementsystem_documents: [{}, {}, {}, {}]
        }
    });

    assert.equal(payload.metadata.app, 'securityguardmanagementsystem');
    assert.equal(payload.collectionCount, 2);
    assert.equal(payload.documentCount, 7);
    assert.deepEqual(payload.collections, [
        { name: 'users', documentCount: 3 },
        { name: 'securityguardmanagementsystem_documents', documentCount: 4 }
    ]);
});
