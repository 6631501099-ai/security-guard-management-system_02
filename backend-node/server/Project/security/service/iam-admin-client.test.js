...
function setScopedGroups() {
  mockServer.state.groups = [
    ...
    },
    {
      ...
      visibleType: { _id: 'type-securityguardmanagementsystem', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] }
    },
    {
      _id: 'group-3',
      title: [{ key: 'en', value: 'SecurityGuardManagementSystem Operator' }],
      visibleType: { _id: 'type-securityguardmanagementsystem', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] }
    }
  ];
}
...
function setScopedMenus() {
  mockServer.state.menus = [
    ...
    },
    {
      ...
      type: { _id: 'type-securityguardmanagementsystem', title: [{ key: 'en', value: 'Security Guard Management System Administration' }] }
    },
    {
...
function setScopedPermissions() {
  mockServer.state.permissions = [
    ...
    {
      _id: 'perm-2',
      name: 'securityguardmanagementsystem.accounts.manage',
    ...
    {
      _id: 'perm-3',
      name: 'securityguardmanagementsystem.lifecycle.manage',
      group: { _id: 'group-2', name: 'Security Guard Management System Admin' },
...
function resetMockState() {
  ...
  mockServer.state.accounts = [{ _id: 'acc-1', email: 'securityguardmanagementsystem.ops@example.com' }];
...
test.before(async function () {
  ...
  AccountModel.onQuery = async function (query) {
    ...
    if (accountId === 'securityguardmanagementsystem-account-1') {
      return {
...
test('iam-admin-client resolves my permissions from securityguardmanagementsystem account email', async function () {
  ...
  mockServer.state.permissions.push({
    _id: 'perm-config',
    name: 'securityguardmanagementsystem.auth-message.manage',
    group: { _id: 'group-2', name: 'Security Guard Management System Admin' },
  ...
  mockServer.state.permissions.push({
    _id: 'perm-training',
    name: 'securityguardmanagementsystem.legacy.manage',
    group: { _id: 'group-2', name: 'Security Guard Management System Admin' },
  ...
  await iamAdminClient.forwardMyPermissions({
    body: { accounts: 'securityguardmanagementsystem-account-1' },
    query: {},
    headers: { lang: 'en' }
  }, response);
  ...
  assert.deepEqual(
    ...
    ['securityguardmanagementsystem.accounts.manage', 'securityguardmanagementsystem.auth-message.manage', 'securityguardmanagementsystem.lifecycle.manage']
  );
  assert.deepEqual(
...
test('iam-admin-client resolves current account and proxies IAM user requests', async function () {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.payload.data.email, 'securityguardmanagementsystem.ops@example.com');
});

test('iam-admin-client allows scoped signin only for accounts inside securityguardmanagementsystem scope', async function () {
  ...
  assert.equal(deniedResponse.payload.error, 'account_not_in_securityguardmanagementsystem_scope');
...
test('iam-admin-client forwards account list and effective permissions', async function () {
  ...
  assert.deepEqual(
    ...
    ['securityguardmanagementsystem.accounts.manage', 'securityguardmanagementsystem.lifecycle.manage']
  );
  assert.deepEqual(
...
test('iam-admin-client lists all securityguardmanagementsystem scoped accounts by default and supports requested pagination', async function () {
  config.security.permissionTypeTitle = 'Security Guard Management System Administration';
  mockServer.state.accounts = [
...
test('iam-admin-client filters account group options to securityguardmanagementsystem permission type', async function () {
...
test('iam-admin-client syncs only securityguardmanagementsystem scoped account assignments to IAM', async function () {
  mockServer.state.accountAssignments = [
    {
      _id: 'acct-assign-1',
      account: 'acc-1',
  ...
  const result = await iamAdminClient.syncAccountAssignments('securityguardmanagementsystem-account-1', ['group-1', 'group-2']);
...
test('iam-admin-client removes only securityguardmanagementsystem scoped account assignments', async function () {
  mockServer.state.accountAssignments = [
    {
      _id: 'acct-assign-securityguardmanagementsystem',
      account: { _id: 'acc-1', email: 'securityguardmanagementsystem.ops@example.com' },
...
test('iam-admin-client enforces scope guard before account mutations', async function () {
  ...
  assert.equal(denyResponse.payload.error, 'account_not_in_securityguardmanagementsystem_scope');
});
...
test('iam-admin-client invites accounts into securityguardmanagementsystem scope only', async function () {
  mockServer.state.accounts = [{ _id: 'acc-1', email: 'securityguardmanagementsystem.ops@example.com' }];
  mockServer.state.accountAssignments = [];
  ...
  assert.equal(invited.account.email, 'new.securityguardmanagementsystem@example.com');
  assert.deepEqual(
  ...
  await iamAdminClient.forwardInviteAccountToScope({
    headers: { lang: 'en' },
    body: {
      email: 'existing.securityguardmanagementsystem@example.com',
      groupIds: ['group-2']
    }
  }, response);
