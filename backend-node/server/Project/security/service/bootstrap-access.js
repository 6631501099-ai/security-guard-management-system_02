...
async function ensureType() {
  ...
  type = await TypeModel.create({
    ...
    description: toLang('Bootstrap type for SecurityGuardManagementSystem administration'),
    state: true
  });
  return type;
}
...
async function ensureGroup(typeId) {
  ...
  group = await GroupModel.create({
    ...
    description: toLang('Bootstrap owner group for SecurityGuardManagementSystem'),
    state: true,
