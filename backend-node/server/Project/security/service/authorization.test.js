...
test.before(async function () {
  ...
  config.iamAdmin.scope = 'security.guard.management.system.registry.read security.guard.management.system.registry.write security.guard.management.system.report.read iam.security.read iam.security.write iam.audit.read iam.accounts.read';
  ...
  AccountModel.onQuery = async function (query) {
    if (query && String(query._id) === 'securityguardmanagementsystem-account-1') {
      return {
...
test('account-access resolves effective permissions from IAM', async function () {
  ...
  const result = await accountAccess.getEffectivePermissions('securityguardmanagementsystem-account-1');
...
test('account-access resolves IAM group options and assignment snapshots', async function () {
  ...
  const assignmentsByAccountId = await accountAccess.loadAssignmentsByAccountIds(['securityguardmanagementsystem-account-1']);

  assert.equal(groups.length >= 2, true);
  assert.equal(groups[0]._id, 'group-1');
  assert.equal(Array.isArray(assignmentsByAccountId['securityguardmanagementsystem-account-1']), true);
  assert.equal(assignmentsByAccountId['securityguardmanagementsystem-account-1'][0].group._id, 'group-1');
});
