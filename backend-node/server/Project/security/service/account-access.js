...
async function findSecurityGuardManagementSystemAccountById(accountId) {
...
async function resolveIamAccountIdBySecurityGuardManagementSystemAccountId(accountId) {
  const securityguardmanagementsystemAccount = await findSecurityGuardManagementSystemAccountById(accountId);
  if (!securityguardmanagementsystemAccount) return String(accountId || '');
  const email = securityguardmanagementsystemAccount && securityguardmanagementsystemAccount.email ? String(securityguardmanagementsystemAccount.email).trim().toLowerCase() : '';
  if (!email) return String(accountId || '');
  const sdk = createSecurityGuardManagementSystemSdk();
...
async function getIamPermissionData(accountId, options) {
  ...
  if (options && options.targetAccountId) {
    targetIamAccountId = await resolveIamAccountIdBySecurityGuardManagementSystemAccountId(options.targetAccountId);
...
async function loadAssignmentsByAccountIds(accountIds) {
  ...
  if (useIamPermissionSource()) {
    const sdk = createSecurityGuardManagementSystemSdk();
    const rows = await Promise.all(ids.map(async function (securityguardmanagementsystemAccountId) {
      try {
        const iamAccountId = await resolveIamAccountIdBySecurityGuardManagementSystemAccountId(securityguardmanagementsystemAccountId);
        if (!iamAccountId) {
          return [String(securityguardmanagementsystemAccountId), []];
        }
        ...
        return [String(securityguardmanagementsystemAccountId), assignments];
      } catch (error) {
        return [String(securityguardmanagementsystemAccountId), null];
      }
    }));
