...
const { createSecurityGuardManagementSystemSdk } = require('../../../integrations/iam/sdk');
...
function createUserRequestOptions(request, options) {
  ...
  return {
    ...
    headers: {
      ...
      'x-system': config.project && config.project.code ? String(config.project.code) : 'securityguardmanagementsystem'
    }
  };
}
...
async function loadCurrentSecurityGuardManagementSystemAccount(request) {
...
async function loadSecurityGuardManagementSystemAccountById(accountId) {
  if (!accountId) return null;
  try {
...
async function resolveIamAccountIdByEmail(email) {
  if (!email) return '';
  ensureAdminConfig();
  const iamSdk = createSecurityGuardManagementSystemSdk();
...
async function resolveIamAccountIdBySecurityGuardManagementSystemAccountId(accountId) {
  const securityguardmanagementsystemAccount = await loadSecurityGuardManagementSystemAccountById(accountId);
  if (!securityguardmanagementsystemAccount) return String(accountId || '');
  const email = securityguardmanagementsystemAccount && securityguardmanagementsystemAccount.email ? securityguardmanagementsystemAccount.email : '';
  ...
  try {
    ...
    if (!allowed) {
      ...
      return response.status(403).json({
        status: false,
        error: 'account_not_in_securityguardmanagementsystem_scope'
      });
    }
...
async function forwardMyPermissions(request, response) {
  try {
    const securityguardmanagementsystemAccount = await loadCurrentSecurityGuardManagementSystemAccount(request);
    const email = securityguardmanagementsystemAccount && securityguardmanagementsystemAccount.email ? securityguardmanagementsystemAccount.email : '';
    if (!email) {
...
async function inviteAccountToScope(payload, headers) {
  ...
  const groupIds = Array.isArray(body.groupIds)
    ...
    : [];
  if (!groupIds.length) {
    const error = new Error('securityguardmanagementsystem_group_required');
    error.statusCode = 400;
    error.payload = { status: false, error: 'securityguardmanagementsystem_group_required' };
    throw error;
  }
...
async function syncAccountAssignments(accountId, groupIds) {
  const iamAccountId = await resolveIamAccountIdBySecurityGuardManagementSystemAccountId(accountId);
  if (!iamAccountId) {
    return {
      accountId: null,
      assignments: [],
...
module.exports = {
  ...
  resolveIamAccountIdBySecurityGuardManagementSystemAccountId,
  requestAdmin,
  requestUser,
  forwardAdminRequest,
