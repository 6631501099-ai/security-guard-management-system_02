...
ควบคุม schema, data contract, migration, seed, index และ compatibility ของ SecurityGuardManagementSystem ให้รองรับ feature ใหม่โดยไม่ทำลาย behavior เดิม.
...
- current models:
  ...
  - `backend-node/server/Project/securityguardmanagementsystem/models/*.js`
...
- controllers/services that normalize payload:
  ...
  - `backend-node/server/Project/securityguardmanagementsystem/service/securityguardmanagementsystem_document.js`
...
## Current SecurityGuardManagementSystem Data Areas

| Domain | Main collections/models |
|---|---|
| Accounts | `accounts/models/account.model.js` |
| SecurityGuardManagementSystem documents | `securityguardmanagementsystem/models/securityguardmanagementsystem_document.model.js` |
...
```txt
ทำหน้าที่ Data Model Agent สำหรับ SecurityGuardManagementSystem
FR: [FR-SecurityGuardManagementSystem-xxx]
...
Constraints:
- ยึด model เดิมของ SecurityGuardManagementSystem
