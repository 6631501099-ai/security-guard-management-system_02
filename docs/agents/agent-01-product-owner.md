...
- current permission paths from `frontend-vue/src/router/index.js`, `backend-node/scripts/bootstrap-securityguardmanagementsystem-permissions.js`, and backend route guards

## Responsibilities

- define goal, business value, in scope, out of scope
- write testable FR IDs เช่น `FR-SecurityGuardManagementSystem-001`
...
## SecurityGuardManagementSystem Product Guardrails
...
- Document registry features must specify whether they affect `/security-guard-management-system/registry`, `/security-guard-management-system/reports`, or both.
...
```txt
ทำหน้าที่ Product Owner สำหรับ SecurityGuardManagementSystem
Requirement: [รายละเอียด]

ช่วยแตกเป็น:
1) Goal
2) In Scope / Out Of Scope
3) FR-SecurityGuardManagementSystem-xxx
