# SecurityGuardManagementSystem Agent Operating Model

อัปเดตจาก source repo `SecurityGuardManagementSystem` ณ วันที่ 2026-06-04

เอกสารชุดนี้คือ operating manual สำหรับใช้ agents พัฒนาและดูแล SecurityGuardManagementSystem. ทุก agent ต้องยึด `docs/AI-WORKFLOW.md`, source code ปัจจุบัน, route mount จริง, frontend route/API จริง และ permission model ที่มีอยู่ ไม่ใช่ prompt ทั่วไป.
...
| `docs/prd/PRD-SecurityGuardManagementSystem.md` | product requirement baseline |
...
|---|---|
| new SecurityGuardManagementSystem feature | Orchestrator, PO, Data Model, Backend, Frontend, Security, QA, Release |
...
- Update `docs/prd/PRD-SecurityGuardManagementSystem.md` when behavior, API, UI, data, permission, test, or release contract changes.
- Run scoped tests/verification before marking implementation done.

## SecurityGuardManagementSystem Source Map For Agents
...
| SecurityGuardManagementSystem document registry | `backend-node/server/Project/securityguardmanagementsystem` | `src/projects/views/securityguardmanagementsystem`, `Service.securityguardmanagementsystemDocuments` |
...
- `orchestrator-example.md`: end-to-end example using the SecurityGuardManagementSystem document registry flow
- `sprint-task-template.md`: reusable task/sprint/handoff template

## Quick Prompt

```txt
ทำงานตาม SecurityGuardManagementSystem agent workflow
Requirement: [describe request]
Source docs:
- docs/AI-WORKFLOW.md
- docs/prd/PRD-SecurityGuardManagementSystem.md
