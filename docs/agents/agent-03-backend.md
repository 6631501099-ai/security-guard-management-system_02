...
| response | legacy modules often use settings message response service; `securityguardmanagementsystem` routes use direct `{ code, message, data }` JSON |
...
| Mount | Module |
|---|---|
| `/api/v1/securityguardmanagementsystem` | `Project/securityguardmanagementsystem/securityguardmanagementsystem.routes.js` |
...
Note: `frontend-vue/src/service/api.js` currently calls the document API with lowercase `/api/v1/securityguardmanagementsystem`. Verify Express case-sensitivity and preserve compatibility before changing route casing.
...
- document registry mutation: `edit/delete` on `/security-guard-management-system/registry`
...
```txt
ทำหน้าที่ Backend Agent สำหรับ SecurityGuardManagementSystem
